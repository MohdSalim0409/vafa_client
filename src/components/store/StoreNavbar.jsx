            ...
            <nav className="fixed top-0 w-full z-50 px-6 md:px-10 py-6">
                <div className="max-w-7xl mx-auto flex justify-between items-center bg-white/70 backdrop-blur-xl border border-white/20 px-8 py-4 rounded-full shadow-sm">
                    <div className="hidden md:flex gap-8 text-[10px] uppercase font-bold tracking-[0.25em] text-gray-800">
                        ...
                        <a href="#story" className="hover:text-neutral-400 transition-colors">
                            The Atelier
                        </a>
                    </div>
                    <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center">
                        <h1 className="text-lg font-serif tracking-[1em] uppercase font-light text-neutral-800 transition-opacity duration-500 hover:opacity-60">VAFAPERFUME</h1>
                    </div>
                                            ...
                                            {authView === "login" ? (
                                                ...
                                                <div className="grid grid-cols-2 gap-x-6 gap-y-6">
                                                    ...
                                                    <div className="col-span-2">
                                                        <InputField label="Address" name="address" onChange={handleSignupChange} />
                                                    </div>
                                                    <div className="col-span-2">
                                                        <InputField label="Password" name="password" type="password" placeholder="••••••••" onChange={handleSignupChange} />
                                                    </div>
                                                </div>
                                            )}
            ...
            <AnimatePresence>
                {showCart && (
                    <>
                        ...
                        {/* Side Drawer */}
                        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 200 }} className="fixed right-0 top-0 h-full w-full max-w-lg bg-white z-[120] shadow-[0_0_50px_rgba(0,0,0,0.1)] flex flex-col">
                            ...
                            <div className="px-10 py-12 flex justify-between items-end">
                                ...
                                <button onClick={() => setShowCart(false)} className="p-2 -mr-2 hover:scale-110 transition-all duration-300 group">
                                        ...
                                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} key={item.sku || idx} className="flex gap-8 items-start group">
                                            ...
                                            <div className="flex-1 flex flex-col min-h-full py-1">
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <h4 className="text-[12px] uppercase tracking-[0.15em] font-semibold text-neutral-900 leading-tight">{item.perfumeName}</h4>
                                                        <p className="text-[9px] text-neutral-400 mt-1.5 tracking-widest font-medium">{item.size}ML / EAU DE PARFUM</p>
                                                    </div>
                                                    <span className="text-[12px] font-light text-neutral-900">${item.priceAtTime?.toFixed(2) || "0.00"}</span>
                                                </div>
                            ...
                            {cartItems.length > 0 && (
                                <div className="px-10 py-10 bg-white border-t border-neutral-50">
                                    ...
                                    <p className="text-center mt-6 text-[9px] text-neutral-400 tracking-widest italic">Complimentary shipping on all artisan orders.</p>
                                </div>
                            )}
                        </motion.div>
                    </>
